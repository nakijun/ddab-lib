{ ##
  ...
  @CREDITS                  + Stefan Winter suggested (and provided prototype
                            ...
                            + Andre Viol suggested that the normal window should
                              ...
                              as required.
                            + Enrico Bortolazzi suggested a method of
                              dynamically instantiating the components. This was
                              adapted when creating the CreateStandAlone
                              constructor.
                            + Bruce J Miller modified TPJRegWdwState to enable
                              reading / writing of additional registry data. He
                              also enabled the components to work with MDI child
                              forms (MDI code later re-written by PJ).
                            + Craig Symons provided code to support multiple
                              monitors.
  @HISTORY(
    @REVISION(
      @VERSION              1.0
    ...
    @REVISION(
      @VERSION              4.3
      @DATE                 04/09/2005
      @COMMENTS             + Added new CreateStandAlone constructor to be used
                              when dynamically creating an instance of
                              TPJWdwState or TPJRegWdwState.
                            + Prevented "Unsafe code" warnings in Delphi 6/7.
    )
    @REVISION(
      @VERSION              4.4
      @DATE                 16/12/2005
      @COMMENTS             Fixed error introduced in v4.3 where code that
      						          switches of "unsafe code" warnings causes unit not
      						          to compile on compilers below Delphi 7.
    )
    @REVISION(
      @VERSION              5.0
      @DATE                 15/10/2006
      @COMMENTS             + Added new TPJUserWdwState component that gets
                              user to provide implementation of reading and
                              writing persistent storage by handling events.
                            + Changed TPJCustomWdwState.OnReadWdwState to
                              protected and exposed in TPJWdwState and
                              TPJRegWdwState - not used in TPJUserWdwState.
    )
    @REVISION(
      @VERSION              5.1
      @DATE                 28/01/2007
      @COMMENTS             Implemented changes coded by Bruce J Miller (BJM)
                            + Added new OnGettingRegData and OnPuttingRegData
                              events that enable additional data to be read from
                              and written to registry.
                            + Implemented support for saving and loading
                              position and size of MDI child forms.
    )
    @REVISION(
      @VERSION              5.2
      @DATE                 19/09/2007
      @COMMENTS             + Implemented changes based on multi-monitor support
                              coded by Craig Symons (CS). Modified
                              TPJCustomWdwState.Restore to automatically adjust
                              size and position for correct monitor if
                              woFitWorkArea in Options.
                            + Removed unused compiler defines.
    )
    @REVISION(
      @VERSION              5.3
      @DATE                 27/09/2007
      @COMMENTS             + Completely re-implemented buggy code that supports
                              fitting MDI child form in parent client workspace.
                            + Bug fix in multi-monitor code by Craig Symons
                              (CS).
                            + Pause before minimization of form now uses a busy
                              wait rather than call to Sleep.
    )
  )
}
